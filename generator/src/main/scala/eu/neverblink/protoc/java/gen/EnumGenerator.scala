...
class EnumGenerator(val info: EnumInfo):
  ...
  def generate =
    ...
    for (value <- info.values.asScala) {
      ...
      t.addField(
        FieldSpec.builder(classOf[Int], EnumGenerator.getValueFieldName(value), PUBLIC, STATIC, FINAL)
  ...
  private def generateStaticMethods(typeSpec: TypeSpec.Builder): Unit =
    ...
    typeSpec.addMethod(MethodSpec.methodBuilder("forNumber")
      .addJavadoc("@param value The numeric wire value of the corresponding enum entry.\n" +
